...
from repositories.file_info_repository import (
    add_file,
    get_files_by_ids,
    delete_file_by_id,
)
...
def upload_file_for_card(
    card_id: str, file_id: str, filename: str, file_bytes: bytes
) -> str:
...
def create_card(card_dto: CardDto) -> Card:
    ...
    if card_dto.title is None or card_dto.title == "":
        raise Exception("Название карточки не может быть пустым")

    if card_dto.description is None:
        card_dto.description = ""

    c = add_card(
        card_dto.title, card_dto.description, card_dto.tags if card_dto.tags else []
    )
    return c
...
def update_card(card_dto: CardDto) -> Card:
    # TODO null checker
    if id is None or id == "":
        raise Exception("ID карточки не может быть пустым")

    res = update_card_by_id(
        card_dto.id,
