...
def add_file(id: str, name: str, audio_file_path: str) -> FileInfo:
...
def get_files_by_ids(file_ids: list[str]) -> list[FileInfo]:
    ...
    res = [
        FileInfo(x["id"], x["name"], x["audio_file_path"], x["file_status"])
        for x in res
    ]
    return res
...
def update_status_for_file(file_id: str, status: str):
    files_table.update_one({"id": file_id}, {"$set": {"file_status": status}})
