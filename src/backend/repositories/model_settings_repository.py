...
def add_model_settings(file_id:str,
    ...
    res = ModelSettings(file_id, window_size, window_step, min_sound_length, ignore_noise_outliers, \
                        ignore_sound_outliers, confidence_limit, offset_bounds)
    return res


def update_default_model_settings(file_id:str,
                       window_size:float = 0.5,
                       window_step:float = 0.1,
                       min_sound_length:float = 0.1,
                       ignore_noise_outliers:str = 'cut_when_at_least_one',
    ...
    model_settings_table.find_one_and_update({'file_id': 'None'}, {'$set': update_dict}, return_document=True)
    res = ModelSettings(file_id, window_size, window_step, min_sound_length, ignore_noise_outliers, \
                        ignore_sound_outliers, confidence_limit, offset_bounds)
    return res
...
def update_model_settings(file_id:str,
    ...
    x = model_settings_table.find_one_and_update({'file_id': file_id}, {'$set': update_dict}, return_document=True)
    res = ModelSettings(file_id, x['window_size'], x['window_step'], x['min_sound_length'], x['ignore_noise_outliers'], \
