...
from services.DbService import model_settings_db
from models.ModelSettings import ModelSettings

def add_model_settings(file_id:str,
                       window_size:float = 0.5,
                       window_step:float = 0.1,
                       min_sound_length:float = 0.33,
                       ...
                       offset_bounds:float = 0.1) -> ModelSettings:
    if file_id is None:
        raise Exception('ERROR file_id is None. For updating default model settings please use update_default_model_settings')
    
    res = model_settings_db.insert_one({
        'file_id': file_id,
...
def update_default_model_settings(window_size:float = 0.5,
                       window_step:float = 0.1,
                       min_sound_length:float = 0.33,
                       ignore_noise_outliers:str = 'cut_when_at_least_one',
                       ignore_sound_outliers:float = 'insert_when_more_than_one',
    ...
    x = model_settings_db.find_one_and_update({'file_id': 'None'}, {'$set': update_dict}, return_document=True)
    res = ModelSettings(window_size, window_step, min_sound_length, ignore_noise_outliers, \
                        ignore_sound_outliers, confidence_limit, offset_bounds)
    return res


def update_model_settings(file_id:str,
                          window_size:float = 0.5,
                          window_step:float = 0.1,
                          min_sound_length:float = 0.33,
                          ignore_noise_outliers:str = 'cut_when_at_least_one',
...
def find_model_settings_by_file_id(file_id:str) -> ModelSettings:
    x = list(model_settings_db.find({'file_id': file_id}))
    if len(x) == 0:
        return None
    x = x[0]
    res = ModelSettings(x['window_size'], x['window_step'], x['min_sound_length'], x['ignore_noise_outliers'], \
...
def find_default_model_settings_by_file_id() -> ModelSettings:
    x = list(model_settings_db.find({'file_id': 'None'}))
    if len(x) == 0:
        return None
    x = x[0]
    res = ModelSettings(x['window_size'], x['window_step'], x['min_sound_length'], x['ignore_noise_outliers'], \
...
def delete_card_by_id(file_id:str):
    model_settings_db.delete_one({'file_id': file_id})