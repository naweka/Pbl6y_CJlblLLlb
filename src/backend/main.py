...
def upgrade_db_check():
    ...
    if not upgrade_res:
        raise Exception("При апгрейде что-то пошло не так...")
...
def prepare_model():
    if not os.path.exists(PATH_TO_THE_MODEL):
        print(
            'Файл модели "/packed_model/unpacked/swin_audio.pth" не был найден! Распаковка...'
        )
        ...
        with open(PATH_TO_THE_PACKED_MODEL_FOLDER + "merged.zip", "wb") as outfile:
            for i in range(1, n + 1):
                filename = (
                    PATH_TO_THE_PACKED_MODEL_FOLDER + zip_prefix + str(i).zfill(3)
                )
                with open(filename, "rb") as infile:
                    outfile.write(infile.read())

        with zipfile.ZipFile(
            PATH_TO_THE_PACKED_MODEL_FOLDER + "merged.zip", "r"
        ) as zip_ref:
            zip_ref.extractall(PATH_TO_THE_MODEL_FOLDER)

    if not os.path.exists(PATH_TO_THE_MODEL):
        raise Exception(
            f"Произошла проблема при распаковке весов модели! Убедитесь, что файл существует: {PATH_TO_THE_MODEL}"
        )
...
def run_event_loop():
    ai_event_loop_thread = Thread(
        target=ai_event_loop, daemon=True, name="ai_event_loop_thread"
    )
